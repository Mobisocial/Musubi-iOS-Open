...
- (void) refreshFriendsIfNeeded {
    ...
    BOOL gConnected = (accounts != nil && accounts.count > 0);
...
- (void) storeContacts: (NSDictionary*) dict {
    ...
    for (NSDictionary* contact in dict) {
        ...
        for (NSDictionary* email in [contact objectForKey:@"gd$email"]) {
            ...
            if (address) {
                MIdentity* mId = [im ensureIdentityWithType:kIdentityTypeEmail andPrincipal:address andName:name ? name : address identityAdded:&_identityAdded profileDataChanged:&_profileDataChanged];
