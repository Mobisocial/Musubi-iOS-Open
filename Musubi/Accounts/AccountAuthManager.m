...
- (void) populateIdentity: (MIdentity*) mIdent withType: (uint8_t) type principal: (NSString*) principal name: (NSString*) name{
    ...
    [mIdent setName: name];
    [mIdent setPrincipal: principal];
    [mIdent setType: type];
    [mIdent setPrincipalHash: hash];
    [mIdent setPrincipalShortHash: *(uint64_t*)hash.bytes];
...
- (BOOL) checkAccount: (NSString*) type name: (NSString*) name principal: (NSString*) principal{
    ...
    if ([type isEqualToString:kAccountTypeFacebook]) {
        authority = kIdentityTypeFacebook;
    } else if([type isEqualToString:kAccountTypeGoogle]) {
        authority = kIdentityTypeEmail;
    } else {
    ...
    @try {
        ...
        MIdentity* mId = [identityManager identityForType:authority andPrincipal:principal];
...
- (MAccount*) storeAccount: (NSString*) type name: (NSString*) name principal: (NSString*) principal{
    ...
    @try {
        ...
        if (mId == nil) {
            ...
            [self populateIdentity:mId withType:authority principal:principal name:name];
